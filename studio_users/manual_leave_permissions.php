...
<head>
    ...
    <title>Manual Leave Permissions | Connect Admin</title>
    ...
    <main class="main-content">
        <header class="dh-nav-header">
            <div class="dh-nav-left" style="display:flex;align-items:center;gap:0.75rem;">
                ...
                <div class="dh-user-info">
                    <div class="dh-icon-orange"><i data-lucide="file-edit" style="width:15px;height:15px;"></i></div>
                    <div class="dh-greeting">
                        <span class="dh-greeting-text">Manual Leave Permission</span>
        ...
        <div class="admin-page-container">
            <div class="admin-card">
                <div class="card-head">
                    <h2 class="card-title">Manual Leave Permissions</h2>
                    <p class="card-sub">Configure who can add manual leave entries for other employees in the Leave Approval dashboard.</p>
                </div>
                <div class="card-body">
                    <div class="toolbar">
                    ...
                    <div id="container" class="grid">
                        <div class="empty"><i class="fa-solid fa-spinner fa-spin" style="font-size:1.3rem;margin-bottom:0.6rem;"></i><p>Loading manual leave permissions...</p></div>
                    </div>

                    <div class="save-wrap">
                        <button id="saveBtn" class="save-btn"><i data-lucide="save" style="width:18px;height:18px;"></i>Save Manual Leave Permissions</button>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function render() {
        ...
        container.innerHTML = filteredUsers.map(u => {
            ...
            const st = state[uid] || { can_add_manual_leave: 0 };
            return `
                <div class="card">
                    <div class="meta">
                    ...
                    <div class="right">
                        <label class="switch-row">
                            ...
                            <span class="switch">
                                <input type="checkbox" class="perm" data-user-id="${uid}" data-key="can_add_manual_leave" ${Number(st.can_add_manual_leave)===1?'checked':''}>
        ...
        document.querySelectorAll('.perm').forEach(cb => {
            cb.addEventListener('change', function() {
                ...
                if (!uid || !key) return;
                if (!state[uid]) state[uid] = { can_add_manual_leave: 0 };
    ...
    async function fetchData() {
        try {
            const res = await fetch('api/get_manual_leave_permissions.php');
            ...
            users.forEach(u => {
                ...
                if (uid > 0) {
                    state[uid] = {
                        can_add_manual_leave: Number(u.can_add_manual_leave || 0) === 1 ? 1 : 0
                    };
                }
            });
            render();
        } catch (e) {
            container.innerHTML = `<div class="empty">Failed to load manual leave permissions.</div>`;
    ...
    async function saveData() {
        ...
        try {
            const res = await fetch('api/save_manual_leave_permissions.php', {
                method: 'POST',
            ...
            showToast('Manual leave permissions saved successfully!');
        } catch (e) {

...
<head>
    ...
    <title>Attendance Action Permissions | Connect Admin</title>
    ...
    <main class="main-content">
        <header class="dh-nav-header">
            <div class="dh-nav-left" style="display:flex;align-items:center;gap:0.75rem;">
                ...
                <div class="dh-user-info">
                    <div class="dh-icon-orange"><i data-lucide="user-check" style="width:15px;height:15px;"></i></div>
                    <div class="dh-greeting">
                        <span class="dh-greeting-text">Attendance Action Permission</span>
        ...
        <div class="admin-page-container">
            <div class="admin-card">
                <div class="card-head">
                    <h2 class="card-title">Attendance Permissions</h2>
                    <p class="card-sub">Configure who can approve, reject, and edit employee attendance/geofence requests.</p>
                </div>
                <div class="card-body">
                    <div class="toolbar">
                    ...
                    <div id="container" class="grid">
                        <div class="empty"><i class="fa-solid fa-spinner fa-spin" style="font-size:1.3rem;margin-bottom:0.6rem;"></i><p>Loading attendance permissions...</p></div>
                    </div>

                    <div class="save-wrap">
                        <button id="saveBtn" class="save-btn"><i data-lucide="save" style="width:18px;height:18px;"></i>Save Attendance Permission</button>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function render() {
        ...
        const rows = users.filter(u => !q || `${u.username||''} ${u.email||''} ${u.role||''}`.toLowerCase().includes(q));
        if (!rows.length) {
        ...
        container.innerHTML = rows.map(u => {
            ...
            const st = state[uid] || { can_approve_attendance: 0, can_reject_attendance: 0, can_edit_attendance: 0 };
            return `
                <div class="card">
                    <div class="meta">
                    ...
                    <div class="right">
                        <label class="switch-row"><span>Approve</span><span class="switch"><input type="checkbox" class="perm" data-user-id="${uid}" data-key="can_approve_attendance" ${Number(st.can_approve_attendance)===1?'checked':''}><span class="slider"></span></span></label>
                        <label class="switch-row"><span>Reject</span><span class="switch"><input type="checkbox" class="perm" data-user-id="${uid}" data-key="can_reject_attendance" ${Number(st.can_reject_attendance)===1?'checked':''}><span class="slider"></span></span></label>
                        <label class="switch-row"><span>Edit</span><span class="switch"><input type="checkbox" class="perm" data-user-id="${uid}" data-key="can_edit_attendance" ${Number(st.can_edit_attendance)===1?'checked':''}><span class="slider"></span></span></label>
                    </div>
                </div>`;
        }).join('');
        ...
        document.querySelectorAll('.perm').forEach(cb => {
            cb.addEventListener('change', function() {
                ...
                if (!uid || !key) return;
                if (!state[uid]) state[uid] = { can_approve_attendance: 0, can_reject_attendance: 0, can_edit_attendance: 0 };
    ...
    async function fetchData() {
        try {
            const res = await fetch('api/get_attendance_action_permissions.php');
            ...
            users.forEach(u => {
                ...
                if (uid > 0) {
                    state[uid] = {
                        can_approve_attendance: Number(u.can_approve_attendance || 0) === 1 ? 1 : 0,
                        can_reject_attendance: Number(u.can_reject_attendance || 0) === 1 ? 1 : 0,
                        can_edit_attendance: Number(u.can_edit_attendance || 0) === 1 ? 1 : 0
                    };
                }
            });
            render();
        } catch (e) {
            container.innerHTML = `<div class="empty">Failed to load attendance permissions.</div>`;
    ...
    async function saveData() {
        ...
        try {
            const res = await fetch('api/save_attendance_action_permissions.php', {
                method: 'POST',
            ...
            showToast('Attendance permissions saved successfully!');
        } catch (e) {

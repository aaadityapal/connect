...
$canAccess = $isAdmin;
if (!$canAccess) {
    $permStmt = $pdo->prepare("SELECT can_access FROM sidebar_permissions WHERE role = ? AND menu_id = 'travel-meter-permissions' LIMIT 1");
...
// All transport modes that the system supports
$allModes = ['Auto', 'Bike', 'Bike Taxi', 'Bus', 'Cab', 'Car', 'E-Rickshaw', 'Flight', 'Metro', 'Other', 'Train'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Travel Meter Photo Permissions | Connect Admin</title>
    ...
    <style>
        ...
        /* ── Legend Bar ── */
        .legend-bar { display: flex; gap: 1rem; align-items: center; flex-wrap: wrap; padding: 0.6rem 1rem; background: #f8fafc; border: 1px solid #eef2f6; border-radius: 10px; margin-bottom: 1rem; font-size: 0.82rem; color: #475569; font-weight: 500; }
        ...
        .modes-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 0.45rem; }
        .mode-row { display: flex; align-items: center; justify-content: space-between; padding: 0.42rem 0.65rem; border: 1px solid #f1f5f9; border-radius: 9px; background: #fafbfc; font-size: 0.82rem; color: #334155; font-weight: 500; gap: 0.5rem; }
        .mode-row:hover { border-color: #e0e7ff; background: #f5f8ff; }
        .mode-icon { font-size: 0.78rem; color: #94a3b8; width: 14px; text-align: center; }
    ...
    <main class="main-content">
            ...
            <div class="dh-nav-left" style="display:flex;align-items:center;gap:0.75rem;">
                ...
                <div class="dh-user-info">
                    ...
                    <div class="dh-greeting">
                        <span class="dh-greeting-text">Travel Meter Photo Permissions</span>
        ...
        <div class="admin-page-container">
            <div class="admin-card">
                <div class="card-head">
                    <h2 class="card-title">Travel Meter Photo Permissions</h2>
                    <p class="card-sub">Control which transport modes require a user to upload meter photos (start &amp; end) when submitting a travel expense.</p>
                </div>
                <div class="card-body">
                    <div class="toolbar">
                    ...
                    <div class="legend-bar">
                        ...
                        <span class="legend-item"><span class="legend-dot dot-on"></span> ON = Meter photo required for this mode</span>
                        <span class="legend-item"><span class="legend-dot dot-off"></span> OFF = No meter photo needed</span>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // All modes the system knows about (injected from PHP)
    const ALL_MODES = <?php echo json_encode($allModes); ?>;

    // Icon map for each transport mode
    const MODE_ICONS = {
        'Auto':       'fa-car-side',
        'Bike':       'fa-bicycle',
        'Bike Taxi':  'fa-motorcycle',
        'Bus':        'fa-bus',
        'Cab':        'fa-taxi',
        'Car':        'fa-car',
        'E-Rickshaw': 'fa-bolt',
    ...
    function render() {
        ...
        container.innerHTML = filtered.map(u => {
            ...
            const modesHtml = ALL_MODES.map(mode => {
                const isOn = (userState[mode] || 0) === 1;
                const icon = MODE_ICONS[mode] || 'fa-circle';
                return `
                <div class="mode-row">
                    <span style="display:flex;align-items:center;gap:6px;">
                        <i class="fa-solid ${icon} mode-icon"></i>
                        ${esc(mode)}
                    </span>
                    <label class="switch">
        ...
        document.querySelectorAll('.perm').forEach(cb => {
            cb.addEventListener('change', function () {
                ...
                state[uid][mode] = this.checked ? 1 : 0;
            });
        });
    ...
    async function fetchData() {
        try {
            const res  = await fetch('api/get_travel_meter_perms.php');
            ...
            // Build state from permissions array returned by API
            users.forEach(u => {
                const uid = Number(u.id) || 0;
                if (!uid) return;
                state[uid] = {};
                ALL_MODES.forEach(m => { state[uid][m] = 0; });
                if (Array.isArray(u.perms)) {
                    u.perms.forEach(p => { state[uid][p.mode] = 1; });
                }
            });

            render();
        } catch (e) {
            container.innerHTML = `<div class="empty">Failed to load permissions: ${esc(e.message)}</div>`;
    ...
    async function saveData() {
        ...
        try {
            const res  = await fetch('api/save_travel_meter_perms.php', {
                method: 'POST',
            ...
            showToast('Meter photo permissions saved successfully!');
        } catch (e) {

...
$records_per_page = 25; // Number of records per page (increased from 10 to 25)
...
if ($week_filter > 0) {
    ...
    if ($month_filter > 0 && $year_filter > 0) {
        ...
        if ($week_filter == 1) {
            ...
            $endDate = clone $firstSunday;
        } else {
            // Calculate start date for week 2+
            $startDate = clone $firstSunday;
            $startDate->modify('+' . (($week_filter - 2) * 7 + 1) . ' days');
        ...
        // Fallback to fixed day ranges if month/year not specified
        if ($week_filter == 1) {

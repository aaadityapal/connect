/**
 * Inventory Upload JavaScript
 * Handles file uploads for inventory bills
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const billFileInputs = document.querySelectorAll('.bill-file-input');
    const billPreviews = document.querySelectorAll('.bill-preview');
    
    // Handle bill file selection
    if (billFileInputs.length > 0) {
        billFileInputs.forEach(function(input, index) {
            input.addEventListener('change', function(e) {
                ...
                if (file) {
                    ...
                    if (preview) {
                        reader.onload = function(e) {
                            preview.innerHTML = `<img src="${e.target.result}" class="bill-preview-img" alt="Bill Preview">`;
                        };
                        reader.readAsDataURL(file);
                    }
                }
    ...
    const inventoryForm = document.getElementById('addEventForm');
    if (inventoryForm) {
    ...
    if (addInventoryBtn) {
        addInventoryBtn.addEventListener('click', function() {
            // This functionality would normally add a new inventory form

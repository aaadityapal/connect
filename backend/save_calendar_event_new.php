...
// For testing - if no session and testing parameters provided, create a test session
$testing_mode = isset($_POST['_testing_mode']) || (isset($_SERVER['HTTP_HOST']) && $_SERVER['HTTP_HOST'] == 'localhost:8000');
if ($testing_mode && !isset($_SESSION['user_id'])) {
    $_SESSION['user_id'] = 1;
    $_SESSION['username'] = 'TestUser';
    $_SESSION['role'] = 'Site Supervisor';
    debug_log('Created test session for development', ['user_id' => $_SESSION['user_id']]);
}
...
/**
 * Process work media file upload
 * @param array $file The uploaded file data
 * @param string $targetDir The directory to save the file to
 * @param int $workId The work ID to associate the file with
 * @param PDO $conn The database connection
 * @return array Result of the operation
 */
function processWorkMediaFile($file, $targetDir, $workId, $conn) {
    debug_log('Processing work media file via processWorkMediaFile()', [
        'work_id' => $workId,
        'file_name' => $file['name'],
        'file_type' => $file['type'],
        'file_size' => $file['size']
    ...
    $modified_file = $file;
    $modified_file['name'] = $unique_name;
    
    // Upload the file to the work-specific directory
    $upload_result = handleFileUpload($modified_file, $targetDir);
    
    if ($upload_result['success']) {
        $file_path = $upload_result['path'];
        $orig_file_name = $file['name']; // Store original filename
        
        debug_log('Work media file uploaded via processWorkMediaFile()', [
            'path' => $file_path,
        ...
        debug_log('Work media record saved to database via processWorkMediaFile()', [
            'media_id' => $media_id
        ]);
        ...
        debug_log('Failed to upload work media file via processWorkMediaFile()', [
...
try {
    ...
    echo json_encode([
        ...
        'line' => $e->getLine(),
        'debug_info' => 'Check logs/calendar_event_errors.log for more details'
    ]);
    
    // Log additional debug information
    debug_log('Additional error details', [
        'POST_data' => $_POST,

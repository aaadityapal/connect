<?php
/**
 * Drop Employee Foreign Key Constraint
 * 
 * This script drops the foreign key constraint on the employee_id column in the overtime_notifications table
 * to prevent similar issues with the payment processing.
 */
...
echo "<h1>Drop Employee Foreign Key Constraint</h1>";
...
if (mysqli_num_rows($constraint_result) == 0) {
    echo "<p style='color:orange'>No foreign key constraint found linking overtime_notifications.employee_id to users.id</p>";
    exit;
}
...
if (mysqli_query($conn, $drop_query)) {
    echo "<p style='color:green'>Successfully dropped employee_id foreign key constraint!</p>";

...
try {
    ...
    $shift_query = "
        SELECT us.shift_id, s.shift_name, s.start_time, s.end_time, us.weekly_offs
        FROM user_shifts us
    ...
    if (!$shift_stmt) {
        ...
        $shift_stmt->bind_param('isss', $user_id, $current_date, $current_date, $current_date);
        $shift_stmt->execute();
        ...
        if ($shift_result && $shift_result->num_rows > 0) {
            ...
            $is_weekly_off = false;
            if (!empty($shift_data['weekly_offs'])) {
                $weekly_offs = explode(',', $shift_data['weekly_offs']);
                if (in_array($day_of_week, $weekly_offs)) {
                    $is_weekly_off = true;
                    // Log that user is punching in/out on a weekly off day
                    error_log("User $user_id is punching $action on a weekly off day");
                }
            }
        }
    ...
    if (!$shift_time) {
        ...
        if ($default_shift_stmt) {
            ...
            if ($default_shift_result && $default_shift_result->num_rows > 0) {
                ...
                $shift_time = $default_shift_data['start_time'];
                
                // Check if today is a weekly off
                $is_weekly_off = false;
                if (!empty($default_shift_data['weekly_offs'])) {
                    $weekly_offs = explode(',', $default_shift_data['weekly_offs']);
                    if (in_array($day_of_week, $weekly_offs)) {
                        $is_weekly_off = true;
                        // Log that user is punching in/out on a weekly off day
                        error_log("User $user_id is punching $action on a weekly off day (default shift)");
                    }
                }
            }
    ...
    if ($action === 'in') {
        ...
        // Calculate lateness with 15-minute buffer
        $lateness_info = '';
        $is_late = false;
        $minutes_late = 0;
        
        if ($shift_time) {
            // Parse shift start time and add 15 minutes for buffer
            $shift_start_timestamp = strtotime($current_date . ' ' . $shift_time);
            $buffer_timestamp = $shift_start_timestamp + (15 * 60); // Add 15 minutes in seconds
            $punch_timestamp = strtotime($timestamp);
            
            if ($punch_timestamp > $buffer_timestamp) {
                // User is late (beyond buffer time)
                $is_late = true;
                $minutes_late = round(($punch_timestamp - $shift_start_timestamp) / 60);
                $lateness_info = "You are $minutes_late minutes late for your shift.";
            } else {
                // User is on time (within buffer)
                $lateness_info = "You've punched in on time.";
            }
        }

function showSubstageFilesDialog(substageId) {
    ...
    fetch(`dashboard/handlers/get_substage_details.php?substage_id=${substageId}`)
        .then(response => response.json())
        .then(detailData => {
            if (!detailData.success) {
                throw new Error(detailData.error || 'Failed to load substage details');
            }
            ...
            // Check if substage is assigned to current user (handle both project and task substages)
            const assignedTo = detailData.substage.assigned_to || detailData.substage.assignee_id;
            
            // If substage has an assigned user and it's not the current user, show error
            if (assignedTo && currentUserId && assignedTo.toString() !== currentUserId.toString()) {
                Swal.fire({
                    icon: 'error',
                    title: 'Access Denied',
                    text: 'This substage is not assigned to you. You cannot perform file operations on it.'
                });
                return;
            }

            // Continue with normal file loading if user has access
        ...
        .catch(error => {
            console.error('Error checking substage assignment:', error);
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: error.message || 'Failed to check substage assignment'
            });
        });
}
...
function addSFComment(substageId) {
    ...
    fetch(`dashboard/handlers/get_substage_details.php?substage_id=${substageId}`)
        ...
        .then(detailData => {
            ...
            // Continue with normal comment/file submission if user has access

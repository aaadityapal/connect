class TaskOverviewManager {
    ...
    async handleForward(e) {
        try {
            ...
            // Debug: Log the event target and its properties
            console.log('Forward click event:', {
                target: e.target,
                classList: e.target.classList ? Array.from(e.target.classList) : [],
                parentElement: e.target.parentElement,
                parentClassList: e.target.parentElement ? Array.from(e.target.parentElement.classList) : []
            });

            // More flexible element selection
            let stageElement = null;
            let substageElement = null;
            let currentElement = e.target;

            // Walk up the DOM tree to find relevant elements
            while (currentElement && !(stageElement && substageElement)) {
                if (currentElement.classList?.contains('stage-item')) {
                    stageElement = currentElement;
                }
                if (currentElement.classList?.contains('substage-item')) {
                    substageElement = currentElement;
                }
            ...
            // Get project ID from data attributes or parent elements
            let projectId = null;
            ...
            for (const container of possibleContainers) {
                if (container?.dataset?.projectId) {
                    projectId = parseInt(container.dataset.projectId, 10);
                    break;
                }
            }

            if (!projectId || isNaN(projectId)) {
                console.error('Project ID not found in containers:', possibleContainers);
            ...
            if (substageElement) {
                const substageId = substageElement.dataset?.substageId;
                if (!substageId) {
                    console.error('Substage element found but no ID:', substageElement);
                ...
                try {
                    ...
                    itemData = {
                        ...itemData,
                        id: parseInt(substageId, 10),
                        stageId: data.substage?.stage_id,
                        title: data.substage?.title || `Substage ${data.substage?.substage_number || 'Unknown'}`,
                        currentAssignee: data.substage?.assigned_to,
                        projectTitle: data.project?.title || 'Unknown Project'
                    };
                } catch (error) {
                    console.error('Error fetching substage details:', error);
                    throw new Error('Failed to load substage information');
                }
            }
            // Handle stage forward
                ...
                if (!stageId) {
                    console.error('Stage element found but no ID:', stageElement);
                ...
                try {
                    ...
                    itemData = {
                        ...
                        title: `Stage ${data.stage?.stage_number || 'Unknown'}`,
                        currentAssignee: data.stage?.assigned_to,
                        projectTitle: data.project?.title || 'Unknown Project'
                    };
                } catch (error) {
                    console.error('Error fetching stage details:', error);
                    throw new Error('Failed to load stage information');
                }
            } else {
                throw new Error('Could not determine if this is a stage or substage forward action');
            }

            // Final validation
            if (!itemData.id || !itemData.projectId || !itemData.title) {
                console.error('Invalid item data:', itemData);
                throw new Error('Missing required information for forward action');
            }

            // Show forward dialog
            return await this.showForwardDialog(itemData);

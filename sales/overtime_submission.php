...
<head>
    ...
    <title>Overtime Submission | Connect</title>
    <!-- Dashboard CSS -->
    <link rel="stylesheet" href="dashboard.css">
    <!-- Load Tailwind CSS (for form components specifically) -->
    ...
    <style>
        :root {
            /* Palette matched from dashboard.css */
            --bg-body: #0a0a0a;
            --bg-card: #161616;
            --text-primary: #ffffff;
        ...
        /* Page Layout matched from Attendance Report */
        .page-header {
            display: flex;
        ...
        /* Utility Overrides for Tailwind conflict */
        .bg-white {
        ...
        .work-report-cell,
        .overtime-report-cell {
            cursor: pointer;
        }

        .work-report-cell:hover,
        .overtime-report-cell:hover {
            background-color: var(--bg-hover);
        ...
        /* Force Input Styling for Modals */
        input,
        select,
        textarea {
        ...
        /* Ensure options in select dropdowns are legible */
        option {
            background-color: var(--bg-card);
            color: var(--text-primary);
        }

        /* Override Tailwind border colors on inputs */
        .border-gray-300 {
            border-color: var(--border-color) !important;
        }
...
<body>
    ...
    <main class="main-content">
        <!-- Top Header matched from Attendance Report -->
        <header class="top-header">
            <button class="mobile-menu-btn" id="mobileMenuBtn">
                <i data-feather="menu"></i>
            </button>
            <div class="page-header-content">
                <h1 class="page-title" style="font-size: 1.25rem;">Overtime Submission</h1>
            </div>
        ...
        <div class="dashboard-view" style="padding-top: 1rem;">
            <!-- 1. Header -->
            <header class="bg-gradient-to-r from-blue-600 to-indigo-700 shadow-lg" style="display: none;">
            ...
            <!-- 2. Filters (Matched to Attendance Report) -->
            <div class="page-header" style="margin-bottom: 1.5rem;">
                <div class="filter-container">
                    <i data-feather="filter" width="16" style="color: var(--text-secondary);"></i>
                ...
                <div class="stats-summary" style="display: flex; gap: 1rem;">
                    <button id="apply-filter-btn" class="punch-btn"
                        style="padding: 0.5rem 1rem; font-size: 0.85rem; height: auto;">
            ...
            <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value" id="pending-count"><?php echo $pending_requests; ?></div>
                ...
                <div class="stat-card">
                    <div class="stat-header">
                        <span class="stat-title">Approved Hours</span>
                        <i data-feather="check-circle" class="stat-icon"></i>
                    </div>
                    <div class="stat-value" id="approved-hours"><?php echo number_format($approved_hours, 1); ?></div>
                    <div class="stat-trend trend-up">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value" id="rejected-requests"><?php echo $rejected_requests; ?></div>
            ...
            <div class="report-card">
                <div class="table-responsive">
                    <table id="overtime-table" class="attendance-table">
                        <thead>
                            <tr>
                                <th>Date</th>
                                <th>End Time</th>
                            ...
                            <?php if (!empty($overtime_data)): ?>
                                <?php foreach ($overtime_data as $index => $row): ?>
                                    <tr id="row-<?php echo $index + 1; ?>"
                                        data-attendance-id="<?php echo isset($row['attendance_id']) ? $row['attendance_id'] : ($index + 1); ?>"
                                        data-overtime-description="<?php echo htmlspecialchars($row['overtime_description'] ?? ''); ?>"
                                        class="hover:bg-gray-50 transition-colors duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?php echo formatDate($row['date']); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?php echo htmlspecialchars($shift_end_time); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            <?php echo htmlspecialchars($row['punch_out_time']); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                            <?php echo htmlspecialchars($row['ot_hours']); ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold"
                                            style="color: var(--text-primary);">
                                            <?php echo !empty($row['accepted_overtime_hours']) ? htmlspecialchars($row['accepted_overtime_hours']) : '-'; ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate work-report-cell"
                                            data-full-text="<?php echo htmlspecialchars($row['work_report']); ?>"
                                            title="<?php echo htmlspecialchars($row['work_report']); ?>">
                                            <?php
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate overtime-report-cell"
                                            data-full-text="<?php echo htmlspecialchars($row['overtime_description']); ?>"
                                            title="<?php echo !empty($row['overtime_description']) ? htmlspecialchars($row['overtime_description']) : 'System deployment and testing'; ?>">
                                            <?php
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            // Check if the record is expired, submitted, approved, or rejected
                                            $is_expired = strtolower($row['status']) === 'expired';
                                            $is_submitted = strtolower($row['status']) === 'submitted';
                                            $is_approved = strtolower($row['status']) === 'approved';
                                            $is_rejected = strtolower($row['status']) === 'rejected';
                                            $is_send_disabled = $is_expired || $is_submitted || $is_approved || $is_rejected;

                                            // Disable edit button for approved and rejected records
                                            $is_edit_disabled = $is_approved || $is_rejected;
                                            $has_valid_overtime_request = !empty($row['attendance_id']) && $row['attendance_id'] > 0;
                                            $edit_button_class = ($has_valid_overtime_request && !$is_edit_disabled)
                                                ? 'action-btn-edit'
                                                : 'action-btn-edit opacity-50 cursor-not-allowed';
                                            $edit_button_disabled = ($has_valid_overtime_request && !$is_edit_disabled) ? '' : 'disabled';
                                            $edit_button_title = ($has_valid_overtime_request && !$is_edit_disabled)
                                                ? 'Edit'
                                                : ($is_edit_disabled ? ($is_approved ? 'Cannot edit approved requests' : 'Cannot edit rejected requests') : 'Cannot edit - No valid overtime request ID');

                                            // Disable all action buttons for Studio Trainees and Site Trainees
                                            $is_trainee = in_array($user_role, ['Studio Trainees', 'Site Trainees']);

                                            // Disable edit button for approved and rejected records, or for trainees
                                            $is_edit_disabled = $is_approved || $is_rejected || $is_trainee;
                                            $has_valid_overtime_request = !empty($row['attendance_id']) && $row['attendance_id'] > 0;
                                            $edit_button_class = ($has_valid_overtime_request && !$is_edit_disabled)
                                                ? 'action-btn-edit'
                                                : 'action-btn-edit opacity-50 cursor-not-allowed';
                                            $edit_button_disabled = ($has_valid_overtime_request && !$is_edit_disabled) ? '' : 'disabled';
                                            $edit_button_title = $is_trainee
                                                ? 'Trainees cannot submit overtime requests'
                                                : (($has_valid_overtime_request && !$is_edit_disabled)
                                                    ? 'Edit'
                                                    : ($is_edit_disabled ? ($is_approved ? 'Cannot edit approved requests' : 'Cannot edit rejected requests') : 'Cannot edit - No valid overtime request ID'));

                                            // Disable send button for expired, submitted, approved, rejected records, or for trainees
                                            $is_send_disabled = $is_expired || $is_submitted || $is_approved || $is_rejected || $is_trainee;
                                            $send_button_class = $is_send_disabled
                                                ? 'action-btn-send opacity-50 cursor-not-allowed'
                                                : 'action-btn-send';
                                            $send_button_disabled = $is_send_disabled ? 'disabled' : '';
                                            $send_button_title = $is_trainee
                                                ? 'Trainees cannot submit overtime requests'
                                                : ($is_expired ? 'Cannot send expired request' : ($is_submitted ? 'Request already submitted' : ($is_approved ? 'Cannot send approved request' : ($is_rejected ? 'Cannot send rejected request' : 'Send/Submit'))));

                                            // Disable resubmit button for non-rejected records or for trainees
                                            $is_resubmit_disabled = !$is_rejected || $is_trainee;
                                            $resubmit_button_class = $is_resubmit_disabled
                                                ? 'action-btn-resubmit opacity-50 cursor-not-allowed'
                                                : 'action-btn-resubmit';
                                            $resubmit_button_disabled = $is_resubmit_disabled ? 'disabled' : '';
                                            $resubmit_button_title = $is_trainee
                                                ? 'Trainees cannot submit overtime requests'
                                                : ($is_resubmit_disabled
                                                    ? ($is_rejected ? 'Resubmit' : 'Can only resubmit rejected requests')
                                                    : 'Resubmit');
                                            ?>
                                            <span
                                                class="status-badge px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $statusClass; ?>">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2 action-buttons">
                                            ...
                                            <button class="action-btn-view" data-action="view"
                                                data-row-id="<?php echo $index + 1; ?>" title="View Details">
                                                <svg class="w-5 h-5 text-blue-600 hover:text-blue-800"
                                            ...
                                            <button class="<?php echo $edit_button_class; ?>" data-action="edit"
                                                data-row-id="<?php echo $index + 1; ?>"
                                                title="<?php echo $edit_button_title; ?>" <?php echo $edit_button_disabled; ?>>
                                                <svg class="w-5 h-5 <?php echo ($has_valid_overtime_request && !$is_edit_disabled) ? 'text-yellow-600 hover:text-yellow-800' : 'text-gray-400'; ?>"
                                                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <button class="<?php echo $send_button_class; ?>" data-action="send"
                                                data-row-id="<?php echo $index + 1; ?>"
                                                title="<?php echo $send_button_title; ?>" <?php echo $send_button_disabled; ?>>
                                                <svg class="w-5 h-5 <?php echo $is_send_disabled ? 'text-gray-400' : 'text-green-600 hover:text-green-800'; ?>"
                                                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <button class="<?php echo $resubmit_button_class; ?>" data-action="resubmit"
                                                data-row-id="<?php echo $index + 1; ?>"
                                                title="<?php echo $resubmit_button_title; ?>" <?php echo $resubmit_button_disabled; ?>>
                                                <svg class="w-5 h-5 <?php echo $is_resubmit_disabled ? 'text-gray-400' : 'text-purple-600 hover:text-purple-800'; ?>"
                                                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div id="confirmation-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-center justify-center p-4 hidden visibility-hidden opacity-0 z-50">
        <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md transform transition-all scale-95 opacity-0"
            ...
            <div class="mt-6 flex justify-end space-x-3">
                <button id="modal-cancel-btn" type="button"
                    class="bg-white px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Cancel
                </button>
                <button id="modal-confirm-btn" type="button"
                    class="text-white px-4 py-2 rounded-lg shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2">
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all scale-95 opacity-0"
            ...
            <div class="bg-gray-50 px-6 py-3 rounded-b-lg">
                <div class="flex justify-end">
                    <button id="text-modal-close-btn" type="button"
                        class="bg-white px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Close
                    </button>
                </div>
            </div>
    ...
    -->
    <div id="overtime-details-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-center justify-center p-4 hidden visibility-hidden opacity-0 z-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all scale-95 opacity-0"
            id="details-modal-content">
            <div class="border-b border-gray-700 px-6 py-4 flex justify-between items-center bg-transparent">
                <h3 class="text-lg font-medium leading-6 text-gray-900 flex items-center">
                    <svg class="h-5 w-5 text-blue-400 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                ...
                <button id="details-modal-close" class="text-gray-400 hover:text-gray-200">
                    <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12" />
            ...
            <div class="px-6 py-4">
                <div class="space-y-4">
                    <div class="flex items-center border-b border-gray-700 pb-2">
                        <svg class="h-4 w-4 text-blue-400 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <span class="text-sm font-medium text-gray-400 w-24">Date</span>
                    ...
                    <div class="flex items-center border-b border-gray-700 pb-2">
                        ...
                        <span class="text-sm font-medium text-gray-400 w-24">End Time</span>
                    ...
                    <div class="flex items-center border-b border-gray-700 pb-2">
                        ...
                        <span class="text-sm font-medium text-gray-400 w-24">Punch Out</span>
                    ...
                    <div class="border-b border-gray-100 pb-2">
                        <div class="flex items-start mb-1">
                            <svg class="h-4 w-4 text-blue-500 mr-2 mt-0.5" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-900 ml-6" id="details-overtime-report">System deployment and testing
                        </p>
                    </div>
                </div>
            </div>
            <div class="bg-gray-50 px-6 py-3 rounded-b-lg">
                <div class="flex justify-end">
                    <button id="details-modal-close-btn" type="button"
                        class="bg-white px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 flex items-center">
                        Close
                    </button>
                </div>
            </div>
    ...
    -->
    <div id="accepted-ot-info-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-center justify-center p-4 hidden visibility-hidden opacity-0 z-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all scale-95 opacity-0"
            id="accepted-ot-info-modal-content">
            ...
            <div class="px-6 py-4">
                <div class="text-sm text-gray-700">
                    <p class="mb-3">
                        You submitted overtime requests with calculated hours, but your manager may have accepted only a
                        portion of those hours.
                    </p>
                    <p class="mb-3">
                        The "Accepted OT Hours" column shows the actual hours that have been approved by your manager.
                        This may differ from the "Calculated OT Hours" based on company policy or other considerations.
                    </p>
                    <p>
                        If there's a discrepancy between calculated and accepted hours, please contact your manager for
                        clarification.
                    </p>
                </div>
            </div>
            <div class="bg-gray-50 px-6 py-3 rounded-b-lg">
                <div class="flex justify-end">
                    <button id="accepted-ot-info-modal-close-btn" type="button"
                        class="bg-white px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Close
                    </button>
                </div>
    ...
    -->
    <div id="send-overtime-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-center justify-center p-4 hidden visibility-hidden opacity-0 z-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all scale-95 opacity-0"
            id="send-modal-content">
            <div class="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                <h3 class="text-lg font-medium leading-6 text-gray-900 flex items-center">
                    <svg class="h-5 w-5 text-indigo-600 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                        fill="currentColor">
            ...
            <div class="px-6 py-4">
                <div class="space-y-4">
                    <div class="flex items-center border-b border-gray-100 pb-2">
                        <svg class="h-4 w-4 text-gray-500 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                    ...
                    <div class="border-b border-gray-100 pb-2">
                        <div class="flex items-start mb-1">
                            <svg class="h-4 w-4 text-gray-500 mr-2 mt-0.5" xmlns="http://www.w3.org/2000/svg"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                    ...
                    <div class="pt-2">
                        ...
                        <textarea id="overtime-description" rows="3"
                            class="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                            placeholder="Describe what you did during overtime..."></textarea>
            ...
            <div class="bg-gray-50 px-6 py-3 rounded-b-lg">
                <div class="flex justify-end space-x-3">
                    <button id="send-modal-cancel-btn" type="button"
                        class="bg-white px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 flex items-center">
                    ...
                    <button id="send-modal-send-btn" type="button"
    ...
    <!-- 
      Overtime Resubmit Modal (Hidden by default)
      Used to resubmit rejected overtime requests.
    -->
    <?php include 'modals/overtime_resubmit_modal.php'; ?>

    <!-- 
      Edit Overtime Modal (Hidden by default)
      Used to edit overtime request details.
    -->
    <div id="edit-overtime-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-center justify-center p-4 hidden visibility-hidden opacity-0 z-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all scale-95 opacity-0"
            ...
            <div class="px-6 py-4">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center border-b border-gray-700 pb-2">
                        ...
                        <span class="text-sm font-medium text-gray-400 w-24">OT Hours</span>
                        <span class="text-sm font-semibold text-gray-900" id="edit-ot-hours">-</span>
                    </div>
                    <div class="border-b border-gray-700 pb-2">
                        <div class="flex items-start mb-1">
                            <svg class="h-4 w-4 text-blue-400 mr-2 mt-0.5" xmlns="http://www.w3.org/2000/svg"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            <span class="text-sm font-medium text-gray-400">Work Report</span>
                        </div>
                        <textarea id="edit-work-report" rows="3"
                            class="block w-full p-2 border border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            placeholder="Enter your work report..."></textarea>
                    </div>

                    <div class="pt-2">
                        <label for="edit-overtime-description"
                            class="block text-sm font-medium text-gray-300 mb-1 flex items-center">
                            <svg class="h-4 w-4 text-blue-400 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        ...
                        <textarea id="edit-overtime-description" rows="3"
                            class="block w-full p-2 border border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                            placeholder="Describe what you did during overtime..."></textarea>
                        <div class="mt-1 text-sm text-gray-500 flex justify-between">
            ...
            <div class="bg-transparent px-6 py-3 rounded-b-lg border-t border-gray-700">
                <div class="flex justify-end space-x-3">
                    <button id="edit-modal-cancel-btn" type="button"
                        class="bg-transparent px-4 py-2 border border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-300 hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 flex items-center transition-colors">
                        <svg class="h-4 w-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                    ...
                    <button id="edit-modal-save-btn" type="button"
    ...
    <!-- 
      Delete Overtime Modal (Hidden by default)
      Used to delete overtime request.
    -->
    <div id="delete-overtime-modal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-center justify-center p-4 hidden visibility-hidden opacity-0 z-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all scale-95 opacity-0"
            id="delete-modal-content">
            <div class="border-b border-gray-200 px-6 py-4 flex justify-between items-center bg-red-50">
                <h3 class="text-lg font-medium leading-6 text-gray-900 flex items-center">
                    <svg class="h-5 w-5 text-red-600 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button id="delete-modal-close" class="text-gray-400 hover:text-gray-500">
                    <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12" />
            ...
            <div class="px-6 py-4">
                ...
                <div class="space-y-4">
                    <div class="flex items-center border-b border-gray-100 pb-2">
                        <svg class="h-4 w-4 text-red-500 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <span class="text-sm font-medium text-gray-500 w-24">Date</span>
                        <span class="text-sm text-gray-900" id="delete-date">-</span>
                    </div>
                    <div class="flex items-center border-b border-gray-100 pb-2">
                        <svg class="h-4 w-4 text-red-500 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <span class="text-sm font-medium text-gray-500 w-24">OT Hours</span>
                        <span class="text-sm font-semibold text-gray-900" id="delete-ot-hours">-</span>
                    </div>
                    <div class="border-b border-gray-100 pb-2">
                        <div class="flex items-start mb-1">
                            <svg class="h-4 w-4 text-red-500 mr-2 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                        ...
                        <p class="text-sm text-gray-900 ml-6" id="delete-work-report">-</p>
                    </div>
                </div>
            </div>
            <div class="bg-gray-50 px-6 py-3 rounded-b-lg">
                <div class="flex justify-end space-x-3">
                    <button id="delete-modal-cancel-btn" type="button"
                        class="bg-white px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 flex items-center">
                        <svg class="h-4 w-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // --- Sidebar Handling is managed by dashboard.js ---
            // Removed redundant minimal sidebar init code
            ...
            function handleFilter() {
                ...
                // Use a safe way to get user ID, fallback to 0 if not set
                const userId = "<?php echo $user_id ?? 0; ?>";

                console.log('Starting filter:', { month, year, userId });
                ...
                const fetchUrl = `../fetch_overtime_data.php?month=${month}&year=${year}&user=${userId}&_t=${timestamp}`;
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            ...
                            if (typeof updateTable === 'function') {
                                ...
                                console.error('updateTable function is defined? no');
                            }
            ...
            function updateTable(data, shiftEndTime) {
                console.log('updateTable called with:', { dataRecords: data?.length, shiftEndTime });
                ...
                if (data && data.length > 0) {
                    ...
                    data.forEach((row, index) => {
                        try {
                            ...
                            tr.dataset.overtimeDescription = row.overtime_report || '';
                            ...
                            // Check if the record has a valid overtime request ID
                            const hasValidOvertimeRequest = row.attendance_id && row.attendance_id > 0;

                            // Check if user is a trainee (Studio Trainees or Site Trainees)
                            const userRoleInput = document.getElementById('current-user-role');
                            const userRole = userRoleInput ? userRoleInput.value : '';
                            const isTrainee = userRole === 'Studio Trainees' || userRole === 'Site Trainees';

                            // Disable edit button for approved and rejected records, or for trainees
                            const isEditDisabled = isApproved || isRejected || isTrainee;
                            const editButtonClass = (hasValidOvertimeRequest && !isEditDisabled)
                                ? 'action-btn-edit'
                                : 'action-btn-edit opacity-50 cursor-not-allowed';
                            const editButtonDisabled = (hasValidOvertimeRequest && !isEditDisabled) ? '' : 'disabled';
                            const editButtonTitle = isTrainee
                                ? 'Trainees cannot submit overtime requests'
                                : ((hasValidOvertimeRequest && !isEditDisabled)
                                    ? 'Edit'
                                    : (isEditDisabled ? (isApproved ? 'Cannot edit approved requests' : 'Cannot edit rejected requests') : 'Cannot edit - No valid overtime request ID'));

                            // Disable send button for expired, submitted, approved, rejected records, or for trainees
                            const isSendDisabled = isExpired || isSubmitted || isApproved || isRejected || isTrainee;
                            const sendButtonClass = isSendDisabled
                                ? 'action-btn-send opacity-50 cursor-not-allowed'
                                : 'action-btn-send';
                            const sendButtonDisabled = isSendDisabled ? 'disabled' : '';
                            const sendButtonTitle = isTrainee
                                ? 'Trainees cannot submit overtime requests'
                                : (isExpired ? 'Cannot send expired request' : (isSubmitted ? 'Request already submitted' : (isApproved ? 'Cannot send approved request' : (isRejected ? 'Cannot send rejected request' : 'Send/Submit'))));

                            // Disable resubmit button for non-rejected records or for trainees
                            const isResubmitDisabled = !isRejected || isTrainee;
                            const resubmitButtonClass = isResubmitDisabled
                                ? 'action-btn-resubmit opacity-50 cursor-not-allowed'
                                : 'action-btn-resubmit';
                            const resubmitButtonDisabled = isResubmitDisabled ? 'disabled' : '';
                            const resubmitButtonTitle = isTrainee
                                ? 'Trainees cannot submit overtime requests'
                                : (isResubmitDisabled
                                    ? (isRejected ? 'Resubmit' : 'Can only resubmit rejected requests')
                                    : 'Resubmit');
                            ...
                            const actionButtons = `
                                ...
                                <button class="${editButtonClass}" data-action="edit" data-row-id="${index + 1}" title="${editButtonTitle}" ${editButtonDisabled}>
                                    <svg class="w-5 h-5 ${(hasValidOvertimeRequest && !isEditDisabled) ? 'text-yellow-600 hover:text-yellow-800' : 'text-gray-400'}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button class="${sendButtonClass}" data-action="send" data-row-id="${index + 1}" title="${sendButtonTitle}" ${sendButtonDisabled}>
                                ...
                                <button class="${resubmitButtonClass}" data-action="resubmit" data-row-id="${index + 1}" title="${resubmitButtonTitle}" ${resubmitButtonDisabled}>
                                    <svg class="w-5 h-5 ${isResubmitDisabled ? 'text-gray-400' : 'text-purple-600 hover:text-purple-800'}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            tr.innerHTML = `
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">${row.ot_hours || '0.0'}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 bg-blue-100">${row.submitted_ot_hours !== 'N/A' ? row.submitted_ot_hours : '-'}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate work-report-cell" data-full-text="${row.work_report || ''}" title="${row.work_report || 'No work report submitted'}">
                                    ${safeTruncate(row.work_report || 'No work report submitted')}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate overtime-report-cell" data-full-text="${row.overtime_report || ''}" title="${row.overtime_report || 'System deployment and testing'}">
                                    ${safeTruncate(row.overtime_report || 'System deployment and testing')}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            console.error('Error creating row:', index, err, row);
                        }
                    });
                } else {
                ...
                try {
                    ...
                    console.warn('Error updating statistics from table:', e);
                }
            }
            ...
            function updateStatisticsFromTable(data) {
                ...
                data.forEach(row => {
                    ...
                    // For approved records, use submitted overtime hours instead of calculated hours
                    let hours = 0;
                    if (status === 'approved' && row.submitted_ot_hours && row.submitted_ot_hours !== 'N/A') {
                        hours = parseFloat(row.submitted_ot_hours) || 0;
                    } else {
                ...
                if (viewButton) {
                    ...
                    if (rowId) {
                        ...
                        if (row) {
                            ...
                            if (status === 'approved') {
                                alert('This overtime request has already been approved and cannot be sent again.');
                                return;
                            }
                        }
                    ...
                    // Check if the button is disabled
                    if (resubmitButton.hasAttribute('disabled')) {
                        // Button is disabled, show tooltip or alert
                        const title = resubmitButton.getAttribute('title');
                        if (title) {
                            alert(title);
                        }
                        return;
                    ...
                    if (rowId) {
                        ...
                        if (row) {
                            ...
                            // Get rejection reason from the status cell
                            let rejectionReason = 'No rejection reason provided';
                            const statusCell = row.cells[7];
                            if (statusCell) {
                                const statusBadge = statusCell.querySelector('.status-badge');
                                if (statusBadge && statusBadge.textContent.trim().toLowerCase() === 'rejected') {
                                    // In a real implementation, you would fetch the actual rejection reason from the database
                                    // For now, we'll use a placeholder
                                    rejectionReason = 'Please review your overtime details and resubmit with corrections.';
                                }
                            }
                            ...
                            if (typeof openOvertimeResubmitModal === 'function') {
                                openOvertimeResubmitModal(resubmitData);
                            } else {
                                console.error('openOvertimeResubmitModal function not found');
                                alert('Resubmit functionality is not properly initialized. Please refresh the page and try again.');
                            }
                        } else {
                            console.error(`Row with ID row-${rowId} not found`);
                        }
    ...
    <script>
        // Sidebar loading logic for Sales Dashboard
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (sidebarContainer) {
                fetch('sidebar.html')
                    ...
                    .then(data => {
                        ...
                        const currentUserName = "<?php echo htmlspecialchars($user_name ?? 'User'); ?>";
                        const currentUserRole = "<?php echo htmlspecialchars($user_role ?? 'Employee'); ?>";
                        ...
                        // --- Initialize Sidebar Toggle Logic (AFTER content is loaded) ---
                        ...
                        if (toggleBtn && sidebar) {
                            toggleBtn.addEventListener('click', () => {
                                ...
                                // Adjust main content margin if needed, though CSS usually handles this via peer-checked or classes
                                // Based on dashboard.css, we might need to toggle a class on the body or main container if it doesn't auto-adjust.
                                // Typically dashboard.css handles .sidebar.collapsed styles.
                            });
                        }

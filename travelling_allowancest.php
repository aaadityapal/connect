...
$employees = [];
try {
    // Query to get active users excluding specific roles
    $query = "SELECT id, username, designation, department, unique_id, role FROM users 
             WHERE deleted_at IS NULL AND status = 'active' 
             AND role NOT IN ('Site Coordinator', 'Site Supervisor', 'Sales', 'Purchase Manager', 'Graphic Designer', 'Social Media Marketing', 'Senior Manager (Studio)', 'Senior Manager (Site)')
...
try {
    // Build the base query with role filtering (excluding specific roles)
    $baseQuery = "FROM travel_expenses te 
                  JOIN users u ON te.user_id = u.id 
                  LEFT JOIN users u_updated ON te.updated_by = u_updated.id
                  WHERE u.role NOT IN ('Site Coordinator', 'Site Supervisor', 'Sales', 'Purchase Manager', 'Graphic Designer', 'Social Media Marketing')";
    ...
    <style>
        ...
        .auto-rejected-badge {
            ...
            margin-right: 0.3rem;
                ...
                /* Custom table wrapper for expenses */
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            if ($isLocked) {
                                                ...
                                                foreach($user_expenses as $exp) {
                                                    if (strtolower($exp['hr_status'] ?? 'pending') === 'pending' || 
                                                        empty($exp['hr_status']) || 
                                                        $exp['hr_status'] === null) {
                                                        $hrPendingCount++;
                                                    }
                                                    
                                                    if (strtolower($exp['accountant_status'] ?? 'pending') === 'pending' || 
                    ...
                    <div class="modal-dialog modal-fullscreen-md-down modal-xxl modal-dialog-centered modal-dialog-scrollable" style="max-width: 95%">
                        <div class="modal-content border-0 shadow">
                            ...
                            <div class="modal-body p-0">
                                ...
                                <div class="modal-expenses-wrapper expenses-table-container" id="expenses-table-<?= $modal_id ?>" style="display:block;">

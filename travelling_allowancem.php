...
<body>
    ...
    <div class="main-content">
                                ...
                                <div class="col-md-6 col-lg-2">
                                    ...
                                    <select class="form-select" id="hr_accountant_status" name="hr_accountant_status">
                                        ...
                                        <?php foreach ($hr_accountant_filter as $value => $label): ?>
                                            <?php
                                            $optionClass = '';
                                            if ($value === 'HR and Accountant Approved')
                                                $optionClass = 'text-success';
                                            elseif ($value === 'HR and Accountant Pending')
                                                $optionClass = 'text-danger';
                                            elseif ($value === 'HR Pending')
                                                $optionClass = 'text-warning';
                                            elseif ($value === 'Accountant Pending')
                                                $optionClass = 'text-warning';
                                            ?>
                                            <option value="<?= htmlspecialchars($value) ?>" class="<?= $optionClass ?>"
                ...
                <div class="card-body p-0">
                    <?php if (!empty($grouped_expenses)): ?>
                                ...
                                <tbody>
                                    ...
                                    foreach ($grouped_expenses as $date => $users_expenses):
                                        foreach ($users_expenses as $user_id => $user_expenses):
                                            ...
                                            $hrApproved = false;
                                            $allRejected = true; // Start with true, will set to false if any expense is not rejected
                                            $totalExpenses = count($user_expenses);
                                            ...
                                            foreach ($user_expenses as $exp) {
                                                ...
                                                $isExpenseRejected = strtolower($exp['status'] ?? '') === 'rejected';

                                                // If any expense is not rejected, set allRejected to false
                                                if (!$isExpenseRejected) {
                                                    $allRejected = false;
                                                }
                                            }
                                            ...
                                            // 2. Today is Wednesday to Saturday (regardless of the travel date)
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <div>
                                                            ...
                                                            $awaitingText = "";
                                                            if ($isLocked) {
                                                                // Count how many expenses have each status
                                                                $hrPendingCount = 0;
                                                                $accountantPendingCount = 0;
                                                                $rejectedCount = 0;

                                                                foreach ($user_expenses as $exp) {
                                                                    if (
                                                                        strtolower($exp['hr_status'] ?? 'pending') === 'pending' ||
                                                                        empty($exp['hr_status']) ||
                                                                        $exp['hr_status'] === null
                                                                    ) {
                                                                        $hrPendingCount++;
                                                                    ...
                                                                    if (
                                                                        strtolower($exp['accountant_status'] ?? 'pending') === 'pending' ||
                                                                        empty($exp['accountant_status']) ||
                                                                        $exp['accountant_status'] === null
                                                                    ) {
                                                                        $accountantPendingCount++;
                                                                    }

                                                                    if (strtolower($exp['status'] ?? '') === 'rejected') {
                                                                        $rejectedCount++;
                                                                    }
                                                                }

                                                                // Build awaiting text based on pending counts
                                                                $pendingParts = [];
                                                                ...
                                                                if ($isLockedDueToWeekday) {
                                                                    ...
                                                                    if ($rejectedCount > 0) {
                                                                        $awaitingText .= " ($rejectedCount rejected)";
                                                                    }
                                                                } else {
                                                                    // Should never reach here with our logic, but just in case
                                                                    $awaitingText = "Awaiting Review";
                                                                }
                                                            }
                                                            ?>
                                                                ...
                                                                <?php if ($additional_expenses > 0): ?>
                                                                    <div>
                                                                        <span class="more-expenses-badge">
                                                                            ...
                                                                            <?= $additional_expenses ?> more
                                                                            expense<?= $additional_expenses > 1 ? 's' : '' ?>
                                                                        </span>
                                                                    </div>
                                                                <?php endif; ?>
                                                                <?php if ($isLocked): ?>
                                                                    <div>
                                                                        <span class="awaiting-badge d-inline-flex"
                                                                            title="This expense requires additional approvals">
                                                                            <?= $awaitingText ?>
                                                                        </span>
                                                                    </div>
                                                                <?php endif; ?>
                                ...
                                <div class="modal fade" id="<?= $modal_id ?>" tabindex="-1" aria-labelledby="<?= $modal_id ?>Label"
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-between">
                                                        ...
                                                        <div class="text-end d-flex flex-column align-items-end">
                                                            <div class="d-flex align-items-baseline">
                                                                ...
                                                                <span
                                                                    class="text-muted ms-2">(<?= number_format($calculatedTotalDistance, 0) ?>
                                                                    km)</span>
                                                            </div>
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                                        ...
                                                        <div class="d-flex align-items-baseline">
                                                            ...
                                                            <h4 class="mb-0 fw-bold text-primary">
                                                                <?= number_format($calculatedTotalDistance, 0) ?>
                                                            </h4>
                                                        ...
                                                        <div class="mt-2 small">
                                                            ...
                                                            <?php if (!empty($expense['hr_confirmed_distance'])): ?>
                                                                <div class="text-info mt-1">
                                                                    <i class="bi bi-check-circle-fill"></i>
                                                                    HR Verified: <?= number_format($expense['hr_confirmed_distance'], 0) ?>
                                                                    km
                                                                    <?php
                                                            ...
                                                            <div class="card h-100 border attendance-photo-card"
                                                                ...
                                                                <div class="card-header bg-primary text-white py-2">
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        <span><i class="bi bi-box-arrow-in-right me-1"></i> Punch
                                                                            In</span>
                                                                        <span
                                                        ...
                                                        <div class="col-md-6">
                                                            <div class="card h-100 border attendance-photo-card"
                                                                ...
                                                                <div class="card-header bg-success text-white py-2">
                                                                    <div class="d-flex justify-content-between align-items-center">
                                                                        <span><i class="bi bi-box-arrow-right me-1"></i> Punch
                                                                            Out</span>
                                                                        <span
                                                                            class="badge bg-light text-dark punch-time">Loading...</span>
                                                                    </div>
                                    ...
                                    `;
                                                        ...
                                                        document.querySelectorAll('.modal').forEach(modal => {
                                                            modal.addEventListener('shown.bs.modal', function () {
                                                                ...
                                                                if (parts.length >= 5) {
                                                                    ...
                                                                    // Check if user is Site Supervisor or Purchase Manager
                                                ...
                                                <div class="table-responsive expenses-table-container"
                                                    id="expenses-table-<?= $modal_id ?>" style="display:block;">
                                                    <table class="table table-hover align-middle mb-0">
                                                        <thead class="table-light">
                                                            <tr>
                                                                <th class="ps-3 text-center" width="40">
                                                                    <div class="form-check">
                                                                        <input class="form-check-input select-all-expenses"
                                                                            type="checkbox" id="selectAll-<?= $modal_id ?>">
                                                                        <label class="form-check-label"
                                                                            for="selectAll-<?= $modal_id ?>"></label>
                                                                    </div>
                                                                </th>
                                                                <th class="ps-3 col-purpose"><i class="bi bi-card-text me-1"></i>
                                                                    Purpose</th>
                                                                <th class="col-route"><i class="bi bi-signpost-split me-1"></i>
                                                                    Route</th>
                                                                <th class="col-mode"><i class="bi bi-truck me-1"></i> Mode</th>
                                                                <th class="col-amount"><i class="bi bi-currency-rupee me-1"></i>
                                                                    Amount</th>
                                                                <th class="col-date"><i class="bi bi-calendar-plus me-1"></i>
                                                                    Submitted</th>
                                                                <th class="col-status"><i class="bi bi-tag me-1"></i> Status</th>
                                                                <th class="col-status"><i class="bi bi-briefcase me-1"></i> Manager
                                                                </th>
                                                                <th class="col-status"><i class="bi bi-calculator me-1"></i>
                                                                    Accountant</th>
                                                                <th class="col-status"><i class="bi bi-people me-1"></i> HR</th>
                                                                <th class="text-end pe-3 col-actions"><i
                                                                        class="bi bi-gear me-1"></i> Actions</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <?php foreach ($user_expenses as $expense):
                                                                ...
                                                                <tr>
                                                                    <td class="ps-3 text-center">
                                                                        <div class="form-check">
                                                                            <input class="form-check-input expense-checkbox"
                                                                                type="checkbox" value="<?= $expense['id'] ?>"
                                                                                id="expense-<?= $expense['id'] ?>"
                                                                                <?= strtolower($expense['status']) !== 'pending' ? 'disabled' : '' ?>>
                                                                            <label class="form-check-label"
                                                                                for="expense-<?= $expense['id'] ?>"></label>
                                                                        </div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="fw-medium">
                                                                        ...
                                                                        <?php if (!empty($expense['notes'])): ?>
                                                                            <div class="text-muted small text-truncate"
                                                                                ...
                                                                                title="<?= htmlspecialchars($expense['notes']) ?>">
                                                                                <?= htmlspecialchars(substr($expense['notes'], 0, 60)) ?>
                                                                                <?= strlen($expense['notes']) > 60 ? '...' : '' ?>
                                                                            </div>
                                                                        <?php endif; ?>
                                                                    </td>
                                                                    <td>
                                                                        <div class="d-flex align-items-center">
                                                                            <span
                                                                                class="badge bg-light text-dark border me-2">From</span>
                                                                            <span class="text-truncate route-text"
                                                                                style="max-width: 180px;"
                                                                                title="<?= htmlspecialchars($expense['from_location']) ?>">
                                                                                <?= htmlspecialchars($expense['from_location']) ?>
                                                                            </span>
                                                                        </div>
                                                                        <div class="d-flex align-items-center mt-1">
                                                                            ...
                                                                            <span class="text-truncate route-text"
                                                                                style="max-width: 180px;"
                                                                                title="<?= htmlspecialchars($expense['to_location']) ?>">
                                                                                <?= htmlspecialchars($expense['to_location']) ?>
                                                                            </span>
                                                                        </div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="d-flex align-items-center">
                                                                            ...
                                                                            ?>
                                                                            <i
                                                                                class="bi bi-<?= $transportIcon ?> me-2 text-primary"></i>
                                                                    ...
                                                                    <td>
                                                                        <span
                                                                            class="badge <?= $statusClass ?>"><?= ucfirst(htmlspecialchars($expense['status'])) ?></span>
                                                                    </td>
                                                                    <td>
                                                                        <div class="d-flex align-items-center">
                                                                            ...
                                                                            <?php if (!empty($expense['rejection_cascade']) && $expense['rejection_cascade'] !== 'MANAGER_REJECTED' && strtolower($expense['manager_status']) === 'rejected'): ?>
                                                                                <span class="badge bg-secondary auto-rejected-badge"
                                                                                    title="Auto-rejected due to rejection by another role">
                                                                    ...
                                                                    <td class="text-end pe-3">
                                                                        <div class="btn-group">
                                                                            ...
                                                                            <?php if (strtolower($expense['status']) == 'pending'): ?>
                                                                                <button class="btn btn-sm btn-outline-success" title="Check"
                                                                                    data-expense-id="<?= $expense['id'] ?>">
                                                                                    <i class="bi bi-check-lg"></i>
                                                                                </button>
                                                                                <button class="btn btn-sm btn-outline-danger" title="Reject"
                                            ...
                                            <div class="modal-footer border-0 bg-light sticky-bottom">
                                                <div class="me-auto">
                                                    <div class="btn-group btn-group-sm flex-wrap"
                                                        id="selected-actions-<?= $modal_id ?>" style="display: none;">
                                                        <span class="me-2 align-self-center selected-count">0 items selected</span>
                                                        <button class="btn btn-success bulk-selected-action"
                                                            data-action="approve-selected">
                                                            <i class="bi bi-check-lg"></i> Check Selected
                                                        </button>
                                                        <button class="btn btn-danger bulk-selected-action"
                                                    ...
                                                    <?php if ($pending_count > 0): ?>
                                                        <div class="btn-group btn-group-sm flex-wrap" id="all-actions-<?= $modal_id ?>"
                                                            style="display: flex;">
                                                            <button class="btn btn-success bulk-action" data-action="approve-all"
                                                                data-user-id="<?= $user_id ?>" data-date="<?= $date ?>">
                                                                <i class="bi bi-check-all"></i> Check All (<?= $pending_count ?>)
                                                            </button>
                                                            <button class="btn btn-danger bulk-action" data-action="reject-all"
                                                                data-user-id="<?= $user_id ?>" data-date="<?= $date ?>">
                                                    ...
                                                    <!-- Footer info message -->
                                                    <div class="text-info" id="footer-message-<?= $modal_id ?>"
                                                        style="display:none;">
                        ...
                        <div class="d-flex justify-content-between align-items-center p-3 border-top">
                            <div class="text-muted small">
                                Showing
                                <strong><?= $offset + 1 ?>-<?= min($offset + count($travel_expenses), $totalRecords) ?></strong>
                                of <strong><?= $totalRecords ?></strong> expenses
                            </div>
                            <?php if ($totalPages > 1): ?>
                ...
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-body">
                        <form id="editExpenseForm">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    <label for="edit-purpose" class="form-label">Purpose of Travel <span
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label for="edit-from-location" class="form-label">From Location <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="edit-from-location" name="from_location"
                                        required>
                                </div>
                                <div class="col-md-6">
                                    <label for="edit-to-location" class="form-label">To Location <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="edit-to-location" name="to_location"
                                        required>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label for="edit-travel-date" class="form-label">Travel Date <span
                                            class="text-danger">*</span></label>
                                    <input type="date" class="form-control" id="edit-travel-date" name="travel_date"
                                        required>
                                </div>
                                <div class="col-md-6">
                                    <label for="edit-mode" class="form-label">Mode of Transport <span
                                            class="text-danger">*</span></label>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" step="0.1" min="0" class="form-control" id="edit-distance"
                                        name="distance">
                                    <small class="form-text text-muted">Distance will be used to calculate amount
                                        automatically for Bike (₹3.5/km) and Car (₹10/km)</small>
                                </div>
                                <div class="col-md-6">
                                    <label for="edit-amount" class="form-label">Amount (₹) <span
                                            class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <input type="number" step="0.01" min="0" class="form-control" id="edit-amount"
                                            name="amount" required>
                                        <button class="btn btn-outline-secondary" type="button"
                                            id="calculate-amount-btn"
                                            title="Calculate amount based on distance and mode">
                            ...
                            <div class="mb-3">
                                ...
                                <div id="current-bill-container" class="mt-2" style="display: none;">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <a href="#" id="current-bill-link" target="_blank"
                                            class="text-decoration-none">View Current Bill</a>
        ...
        <!-- Photo Viewer Modal -->
        <div class="modal fade" id="photoViewerModal" tabindex="-1" aria-labelledby="photoViewerModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
        ...
        <div class="modal fade" id="confirmActionModal" tabindex="-1" aria-labelledby="confirmActionModalLabel"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="confirmActionForm">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="invalid-feedback" id="reason-error">Please provide a detailed reason (at
                                    least 10 words).</div>
                            </div>
                            ...
                            <div class="mb-3 border-top pt-3">
                                ...
                                <div class="form-check mb-2">
                                    <input class="form-check-input verification-checkbox" type="checkbox"
                                        id="check-destination" name="check_destination" required>
                                ...
                                <div class="form-check mb-2">
                                    <input class="form-check-input verification-checkbox" type="checkbox"
                                        id="check-policy" name="check_policy" required>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" id="cancel-action-btn"
        ...
        <div class="modal fade" id="statusConfirmationModal" tabindex="-1"
            aria-labelledby="statusConfirmationModalLabel" aria-hidden="true" style="z-index: 1060; box-shadow: none;">
            <div class="modal-dialog modal-dialog-centered" style="box-shadow: none;">
                <div class="modal-content" style="box-shadow: none;">
                    <div class="modal-header" id="statusConfirmationHeader">
                        ...
                        <button type="button" class="btn-close" id="status-modal-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" id="status-modal-btn"
                            data-bs-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>

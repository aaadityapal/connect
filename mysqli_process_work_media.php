...
try {
    ...
    require_once 'includes/work_progress_media_handler_mysqli.php';
    ...
    // Check if work progress ID exists in the database
    $checkResult = $conn->query("SELECT id FROM work_progress WHERE id = $workProgressId");
    if (!$checkResult || $checkResult->num_rows === 0) {
        error_log("Warning: Work progress ID $workProgressId doesn't exist in the database");
        // We'll continue anyway, but log the warning
    }
    
    // Check for upload directory
    $uploadDir = dirname(__FILE__) . '/uploads/work_progress/';
    ...
    $mediaHandler = new WorkProgressMediaHandler($conn);

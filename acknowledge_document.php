...
try {
    ...
    $checkStmt = $pdo->prepare("SELECT id FROM document_acknowledgments WHERE document_id = ? AND user_id = ?");
    ...
    if ($existing) {
        ...
        $insertStmt = $pdo->prepare("INSERT INTO document_acknowledgments 
            (document_id, user_id, acknowledged_at, created_at) 
            VALUES (?, ?, NOW(), NOW())");
    ...
    echo json_encode([
        'success' => false,
        'message' => 'Error acknowledging document: ' . $e->getMessage()
    ]);
} 
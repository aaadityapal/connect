...
<html>
<head>
    <title>Timer Test for Missing Punches Modal</title>
    <meta charset="UTF-8">
...
<body>
    <div class="container">
        <h1>Timer Test for Missing Punches Modal</h1>
        
        <div class="info-box">
            <p>This page demonstrates the timer functionality in the missing punches modal.</p>
            <p>The timers update in real-time to show when each missing punch will be marked as absent.</p>
        </div>
        
        <div class="timer-demo">
            <p>Current time: <span id="currentTime" class="timer-value">Loading...</span></p>
            <p>Next update in: <span id="nextUpdate" class="timer-value">10</span> seconds</p>
        </div>
    ...
    <script>
        ...
        function updateCurrentTime() {
            ...
            // Update timers in modal if it's open
            updateTimers();
        }
        
        // Update all timers
        function updateTimers() {
            const timerElements = document.querySelectorAll('.missing-punch-timer:not(.expired)');
            timerElements.forEach(element => {
                // Get the punch date from the parent element
                const punchItem = element.closest('.missing-punch-item');
                if (punchItem) {
                    // In a real implementation, we would get the actual date from the data
                    ...
                    element.textContent = `Submit within ${randomHours}h ${randomMinutes}m to avoid being marked absent`;
                    
                    // Update class based on time remaining
                    element.className = 'missing-punch-timer';
                    if (randomHours < 6) {
        ...
        function updateCountdownDisplay() {
            document.getElementById('nextUpdate').textContent = updateCountdown;
            updateCountdown--;
        ...
        function showTestModal() {
            ...
            if (modal) {
                ...
                if (typeof fetchMissingPunches === 'function') {
                    ...
                    // Use mock data if the real function isn't available
                    fetchMissingPunchesMock();
                }
            }
        }
        
        // Mock function to fetch missing punches data
        function fetchMissingPunchesMock() {

...
try {
    ...
    // Query to check which dates have pending missing punch submissions
    $submitted_query = "
        SELECT date, 'in' as type FROM missing_punch_in 
        WHERE user_id = ? AND date IN ($placeholders) AND status = 'pending'
        UNION
    ...
    // Create array of submitted dates
    $submitted_dates = [];
    foreach ($submitted_records as $record) {
        $submitted_dates[] = $record['date'];
    }

...
if (isset($_GET['action']) && $_GET['action'] === 'get_vendor_labour_data') {
    ...
    // Debug information
    error_log("Autocomplete request - Type: $data_type, Search term: $search_term, User ID: $user_id");
    ...
    if ($data_type === 'vendor') {
        ...
        // Debug information
        error_log("Vendor query found " . $result->num_rows . " results");
    ...
    elseif ($data_type === 'vendor_labour') {
        ...
        // Debug information
        error_log("Labour search - Vendor Type: $vendor_type, Vendor Name: $vendor_name");
        ...
        // Debug information
        error_log("Labour query found " . $result->num_rows . " results");
    ...
    elseif ($data_type === 'company_labour') {
        // Debug information
        error_log("Company labour search - Term: $search_term");
        ...
        // Debug information
        error_log("Combined labour query found " . $result->num_rows . " results");
            ...
            foreach ($_POST['vendors'] as $vendor_id => $vendor_data) {
                ...
                if (isset($vendor_data['labours']) && is_array($vendor_data['labours'])) {
                    foreach ($vendor_data['labours'] as $labour_id => $labour_data) {
                        ...
                        $labour_query = "INSERT INTO vendor_labours (vendor_id, labour_name, mobile, morning_attendance, afternoon_attendance, ot_hours, ot_wages, wage, ot_amount, total_amount) 
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                        $labour_stmt = $conn->prepare($labour_query);
                        $labour_stmt->bind_param("issssddddd", $vendor_db_id, $labour_name, $labour_mobile, $labour_attendance, $labour_attendance, $labour_ot_hours, $labour_ot_amount, $labour_wage, $labour_ot_amount, $labour_total);
                        $labour_stmt->execute();
                    }
                }
            }
...
<body>
    ...
    <script>
        ...
        function initLabourAutocomplete(labourId, vendorId) {
            console.log('Initializing labour autocomplete for labourId: ' + labourId + ', vendorId: ' + vendorId);
            ...
            $(labourNameInput).autocomplete({
                source: function(request, response) {
                    console.log('Labour search request:', request.term);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            console.log('Labour search response:', data);
                            if (data.success) {
                                response($.map(data.data, function(item) {
                                    return {
                                        label: item.name,
        ...
        function fetchSiteUpdateDetails(siteName, date) {
            ...
            fetch(`site_expenses_updated.php?action=get_site_update_details&site_name=${encodeURIComponent(siteName)}&date=${encodeURIComponent(date)}`)
                .then(response => {
                    if (!response.ok) {
        ...
        function initCompanyLabourAutocomplete(labourId) {
            console.log('Initializing company labour autocomplete for labourId: ' + labourId);
            ...
            $(labourNameInput).autocomplete({
                source: function(request, response) {
                    console.log('Company labour search request:', request.term);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            console.log('Company labour search response:', data);
                            if (data.success) {
                                response($.map(data.data, function(item) {
                                    return {
                                        label: item.name,
                        ...
                        error: function(xhr, status, error) {
                            console.error('Company labour search error:', error);
                            response([]);
                        }
                    });
                },

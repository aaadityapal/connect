...
<div class="compact-time-widget <?php echo $greeting_class; ?>">
    ...
    <div class="user-controls-container">
        <!-- Punch-In Button -->
        <button id="punchInButton"
            class="punch-in-button <?php echo $greeting_class; ?> <?php echo $is_punched_in ? 'punched-in' : ''; ?> <?php echo $attendance_completed ? 'completed' : ''; ?>"
    ...
    <div class="punch-modal-content">
        ...
        <div class="punch-modal-body">
            ...
            <div class="location-info">
                ...
                <div id="outsideReasonContainer" class="outside-reason-container" style="display:none;">
                    <div class="reason-header">
                        ...
                        <span>You are outside the allowed area. Please provide a reason for punching in remotely (max 15
                            words):</span>
                    </div>
                    <textarea id="outsideReason" class="outside-reason-input" maxlength="100"
                        placeholder="Enter reason for outside location (required)"></textarea>
                    <div class="word-counter"><span id="wordCount">0</span>/15 words</div>
                    <div class="reason-note"><i class="fas fa-info-circle"></i> Note: Special characters alone won't be
                        counted as words.</div>
                </div>
            </div>
            ...
            <div id="workReportContainer" class="work-report-container" style="display:none;">
                ...
                <textarea id="workReport" class="work-report-input" maxlength="500"
                    placeholder="Describe tasks completed, progress made, and challenges faced today..."></textarea>
...
<style>
    ...
    @keyframes flame-flicker {
        ...
        100% {
            box-shadow: 0 0 15px #FF6347, 0 0 25px #FF4500, 0 0 30px #FF4500;
        }
    }
    ...
    .compact-time-widget {
        ...
        /* Add extra padding for string lights */
        margin: 15px 0;
        ...
        align-items: center;
        overflow: visible;
        /* Allow dropdowns to extend beyond boundaries */
    }
    ...
    .punch-in-button {
        ...
        /* Golden gradient for Diwali */
        color: #8B0000;
        /* Dark red text for contrast */
        position: relative;
        overflow: hidden;
        border: 2px solid #FF4500;
        /* Diwali orange border */
    }
    ...
    @media (max-width: 480px) {
        ...
        .outside-reason-container {
            padding: 8px;
            padding-bottom: 20px;
            /* Add extra padding at bottom for the note */
        }
    ...
    @media (max-width: 375px) {
        ...
        .punch-in-button {
            top: 85px;
            /* Adjusted from 55px to 85px to match the change above */
            padding: 6px 15px;
            font-size: 0.8rem;
            min-width: 100px;
        }
    ...
    #cameraFeed {
        min-height: 200px;
        /* Smaller camera view on very small screens */
    }
    ...
    @media (max-width: 768px) {
        ...
        .outside-reason-input {
            min-height: 100px;
            font-size: 16px;
            /* Larger font for better mobile input */
            padding: 12px;
            border-width: 2px;
        }
    ...
    .work-report-container {
        ...
        position: relative;
        /* Added for proper positioning of elements */
    }
    ...
    .shift-timer .timer-label {
        ...
        font-size: 14px;
        margin-right: 6px;
        /* Add margin to separate label from value */
        margin-bottom: 0;
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function checkNotificationReadStatus(missingPunches) {
            ...
            if (dates.length === 0) {
                return Promise.resolve({ read_dates: [], submitted_punch_in_dates: [], submitted_punch_out_dates: [] });
            }
                ...
                .then(data => {
                    if (data.success) {
                        return {
                            ...
                            submitted_punch_in_dates: data.submitted_punch_in_dates || [],
                            submitted_punch_out_dates: data.submitted_punch_out_dates || []
                        };
                    } else {
                        console.error('Error checking read status:', data.message);
                        return { read_dates: [], submitted_punch_in_dates: [], submitted_punch_out_dates: [] };
                    }
                })
                .catch(error => {
        ...
        function updateNotificationList(missingPunches, statusData = { read_dates: [], submitted_punch_in_dates: [], submitted_punch_out_dates: [] }) {
            const { read_dates = [], submitted_punch_in_dates = [], submitted_punch_out_dates = [] } = statusData;
            ...
            missingPunches.forEach(punch => {
                ...
                // Check if this is a punch-in or punch-out notification
                if (punch.type === 'punch_in') {
                    isSubmitted = submitted_punch_in_dates.includes(punch.date);
                    isRead = read_dates.includes(punch.date) && !isSubmitted;
                } else if (punch.type === 'punch_out') {
                    isSubmitted = submitted_punch_out_dates.includes(punch.date);
                    isRead = read_dates.includes(punch.date) && !isSubmitted;
                }
            ...
            function markNotificationAsRead(date) {
                ...
                .then(data => {
                    if (data.success) {
                        console.log('Notification marked as read for date: ' + date);
                    } else {
            ...
            document.querySelectorAll('.notification-item').forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    // Mark notification as read when clicked (if not already submitted)
                    const isSubmitted = this.querySelector('.notification-status.submitted');
                    if (!isSubmitted) {

...
<body>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Save all expenses from modal
            $('#saveAllExpenses').on('click', function() {
                // Get all expenses from the modal
                const modalExpenses = [];
                $('.travel-expenses-list .expense-entry').each(function() {
                    const entryId = $(this).data('entry-id');
                    const purpose = $(this).find('.detail-value[data-field="purpose"]').text();
                    const mode = $(this).find('.detail-value[data-field="mode"]').text();
                    const from = $(this).find('.detail-value[data-field="from"]').text();
                    const to = $(this).find('.detail-value[data-field="to"]').text();
                    
                    // Get date from the original-date data attribute if available, otherwise from the text
                    const dateElement = $(this).find('.detail-value[data-field="date"]');
                    let dateValue = dateElement.data('original-date') || dateElement.text();
                    let formattedDate = formatDateToYYYYMMDD(dateValue);
                    
                    // Get distance and amount, removing any non-numeric characters
                    const distanceText = $(this).find('.detail-value[data-field="distance"]').text().replace(/[^\d.]/g, '');
                    const amountText = $(this).find('.detail-value[data-field="amount"]').text().replace(/[^\d.]/g, '');
                    
                    const distance = parseFloat(distanceText) || 0;
                    const amount = parseFloat(amountText) || 0;
                    const notes = $(this).find('.entry-notes').text();
                    ...
                    modalExpenses.push({
                        ...
                        status: status,
                        notes: notes,
                        billFileIndex: entryId
                    });
                });
                
                if (modalExpenses.length === 0) {
                    alert('Please add at least one travel expense entry.');
                    return;
                }
                    ...
                    if (this.status === 200) {
                        try {
                            ...
                            if (response.success) {
                                ...
                                alert('Error saving expenses: ' + (response.message || 'Unknown error'));
                            }
                        } catch (e) {
                            console.error('Error parsing response:', e);
                ...
                const data = 'expenses=' + encodeURIComponent(JSON.stringify(modalExpenses));
                xhr.send(data);
            });
